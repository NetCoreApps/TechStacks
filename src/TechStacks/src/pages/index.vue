                  ...
                  <v-flex style="text-align:center">
                      <v-card flat color="transparent">
                        <p>
                          ...
                          <nuxt-link to="/techstacks-website">TechStacks was created</nuxt-link> with.
                        </p>
                        <p>
        ...
        <v-flex xs8>
          <v-layout row wrap>
            <v-flex xs6 
              ...
              :key="tier.name"
            >
              <nuxt-link :to="`/tech?tier=${tier.name}`">
              ...
              <v-card>
                      ...
                      <v-flex>
                        <v-flex v-for="tech in topTechnologiesByTier(tier.name)" :key="tech.name">
                          <v-layout align-center>
                            ...
                            <v-flex>
                              <nuxt-link :to="`/tech/${tech.slug}`"><img :src="tech.logoUrl" height="44"></nuxt-link>
                            </v-flex>
                          </v-layout>
                        </v-flex>
                      </v-flex>
                ...
                <v-card-actions class="white">
                  <v-spacer></v-spacer>
                  <v-btn flat :to="`/tech?tier=${tier.name}`">view all</v-btn>
                </v-card-actions>
              </v-card>
            </v-flex>
          </v-layout>
                    ...
                    <v-layout row>
                      <v-flex>
                        <v-flex class="headline" v-for="tech in topTechnologies" :key="tech.name">
                          <nuxt-link :to="`/tech/${tech.slug}`"><em>({{ tech.stacksCount }})</em> {{ tech.name }}</nuxt-link>
                        </v-flex>
                      </v-flex>
                    </v-layout>
                  </v-container>
                    ...
                    <v-layout row>
                      <v-flex>
                        <v-flex class="headline" v-for="user in topUsers" :key="user.userName">
                          <nuxt-link :to="`/users/${user.userName}`"><em>({{ user.stacksCount }})</em> {{ user.userName }}</nuxt-link>
                        </v-flex>
                      </v-flex>
                    </v-layout>
                  </v-container>
